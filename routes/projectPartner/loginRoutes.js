...
router.post("/login", async (req, res) => {
  try {
    ...
    // Generate JWT Token
    const token = jwt.sign({ id: user.id, username: user.username, contact: user.contact, email: user.email, adharId: user.adharno, freeProjectPartner: user.freeProjectPartner }, process.env.JWT_SECRET, {
      expiresIn: "10d",
    });
...
router.post("/login/forgot-password", async (req, res) => {
  try {
    ...
    const user = await new Promise((resolve, reject) => {
      db.query(
        "SELECT * FROM projectpartner WHERE email = ?",
        [email],
        (err, results) => {
          if (err) {
    ...
    await new Promise((resolve, reject) => {
      db.query(
        "UPDATE projectpartner SET password = ? WHERE email = ?",
