...
export const token = async (req, res) => {
  ...
  try {
    // STEP 1: Upload payment image to S3 (compressed if image)
    ...
    if (req.file) {
      ...
      paymentImageUrl = await uploadToS3(uploadFile);
    }
      ...
      (err, enquirerResult) => {
        ...
        db.query(
          ...
          (err, propertyResult) => {
            ...
            let commissionType = property.commissionType || "";
            let commissionPercentage = Number(property.commissionPercentage) || 0;
            let finalCommissionAmount =
              Number(property.commissionAmount) || 0;
            ...
            const grossSalesCommission =
              (finalCommissionAmount * 40) / 100;
            const salesCommission =
              grossSalesCommission -
              (grossSalesCommission * 2) / 100;
            const grossTerritoryCommission =
              (finalCommissionAmount * 20) / 100;
            ...
            // STEP 4: Insert Followup
            const insertSQL = `
              INSERT INTO propertyfollowup (
                enquirerid, paymenttype, tokenamount, remark, dealamount, status,
                totalcommission, reparvcommission, salescommission, territorycommission, tds,
                paymentimage, updated_at, created_at
              ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `;
