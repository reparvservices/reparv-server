...
export const add = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      if (result.length === 0) {
        const insertSQL = `INSERT INTO builders (builderadder, company_name, contact_person, contact, email, uid, office_address, registration_no, dor, website, experience,
          about,
          vision,
          mission,
          quality,
          expertise,
          why_choose, notes, updated_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
