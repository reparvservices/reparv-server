...
//       return res
//         .status(400)
//         .json({ message: "Full name and contact and Email are required." });
//     }

//     // Step 1: Check if contact already exists
//     const checkSql =
//       "SELECT user_id, fullname, contact FROM mobileusers WHERE contact = ? OR email = ?";
//     db.query(checkSql, [contact,email], (checkErr, checkResults) => {
//       if (checkErr) {
...
//       const insertSql = `INSERT INTO mobileusers (fullname, contact,email, updated_at, created_at) VALUES (?, ?,?, ?, ?)`;
//       db.query(
//         insertSql,
//         [fullname, contact,email, currentdate, currentdate],
//         (insertErr, result) => {
//           if (insertErr) {
