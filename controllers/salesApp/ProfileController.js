...
export const sendRequest = async (req, res) => {
  try {
    const { salespersonid, territoryid, projectpartnerid, requestreason } =
      req.body;
    ...
    db.query(
      sql,
      [salespersonid, projectpartnerid, requestreason],
      (err, result) => {
        if (err) {
...
export const updateProfileHeader = async (req, res) => {
  ...
  const sql = `UPDATE salespersons SET ${fields.join(", ")} WHERE salespersonsid = ?`;
