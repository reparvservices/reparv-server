...
export const addPropertyOld = async (req, res) => {
  ...
  try {
    // Upload each image to S3 and convert to URL
...
export const v2Update = async (req, res) => {
  ...
  // Validation (keep minimal required)
  if (!propertyCategory || !propertyName || !address || !state || !city) {
  ...
  if (possessionDate && moment(possessionDate).isValid()) {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      // Helper: merge old + new images
      const mergeImages = (existingField, newField) => {
        try {
          const oldImages = JSON.parse(existingField || "[]");
          return JSON.stringify(newField.length > 0 ? newField : oldImages);
        } catch {
          return JSON.stringify(newField || []);
        }
      };
      ...
      try {
        ...
        const values = [
          ...
          // image handling
          mergeImages(existing.frontView, frontView),
          mergeImages(existing.sideView, sideView),
          mergeImages(existing.kitchenView, kitchenView),
          mergeImages(existing.hallView, hallView),
          mergeImages(existing.bedroomView, bedroomView),
          mergeImages(existing.bathroomView, bathroomView),
...
// * UPLOAD Brochure & Video Link to S3 *
  ...
  try {
    ...
    db.query(
      ...
      async (err, result) => {
        ...
        // Upload new images to S3 if provided, else keep old paths
        const getImagePaths = async (field) => {
          if (files[field]) {
            const urls = [];
            for (const file of files[field]) {
            ...
            return existing[field]; // keep old S3 URLs or local paths
          }
        };

        // Prepare all image fields
        const frontView = await getImagePaths("frontView");
        const sideView = await getImagePaths("sideView");
        const kitchenView = await getImagePaths("kitchenView");
        const hallView = await getImagePaths("hallView");
    ...
    return res
      .status(500)
      .json({ message: "File conversion/upload failed", error: err });
  }
};
  ...
  try {
    ...
    db.query(
      ...
      async (err, result) => {
        ...
        // Delete images from S3 or filesystem
        await Promise.all(
