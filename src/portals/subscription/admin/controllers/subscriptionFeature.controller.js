...
const insertFeatureRow = async (name, status, description) => {
  ...
  try {
    const [result] = await dbPromise.query(
      "INSERT INTO subscription_features (name, description, status) VALUES (?, ?, ?)",
    ...
    if (e?.code === "ER_BAD_FIELD_ERROR") {
      const [result] = await dbPromise.query(
        "INSERT INTO subscription_features (name, status) VALUES (?, ?)",
        [name.trim(), status],
      );
      return result;
    }
...
const updateFeatureRow = async (id, name, status, description) => {
  ...
  try {
    await dbPromise.query(
      "UPDATE subscription_features SET name = ?, description = ?, status = ? WHERE id = ?",
      [name.trim(), desc, status, id],
    );
  } catch (e) {
    if (e?.code === "ER_BAD_FIELD_ERROR") {
      await dbPromise.query("UPDATE subscription_features SET name = ?, status = ? WHERE id = ?", [
        name.trim(),
        status,
        id,
      ]);
...
export const getAllFeatures = async (req, res) => {
  try {
    const [rows] = await dbPromise.query("SELECT * FROM subscription_features ORDER BY id DESC");
...
export const getFeatureById = async (req, res) => {
  try {
    ...
    const [rows] = await dbPromise.query("SELECT * FROM subscription_features WHERE id = ?", [id]);
...
export const createFeature = async (req, res) => {
  try {
    ...
    const [duplicates] = await dbPromise.query(
      "SELECT id FROM subscription_features WHERE LOWER(name) = LOWER(?)",
      [name.trim()],
    );
...
export const updateFeature = async (req, res) => {
  try {
    ...
    const [rows] = await dbPromise.query("SELECT id FROM subscription_features WHERE id = ?", [id]);
    if (!rows.length) return res.status(404).json({ message: "Feature not found" });

    const [duplicates] = await dbPromise.query(
      "SELECT id FROM subscription_features WHERE LOWER(name) = LOWER(?) AND id != ?",
      [name.trim(), id],
    );
    if (duplicates.length) {
      return res.status(409).json({ message: "Feature already exists" });
...
export const deleteFeature = async (req, res) => {
  try {
    ...
    await dbPromise.query("DELETE FROM subscription_features WHERE id = ?", [id]);
