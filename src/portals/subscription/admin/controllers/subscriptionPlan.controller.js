...
export const getAllPlans = async (_req, res) => {
  try {
    const [rows] = await dbPromise.query(
      ...
      LEFT JOIN subscription_features sf ON sf.id = pfm.feature_id
      GROUP BY sp.id
      ORDER BY sp.id DESC`,
    );
  ...
  try {
    ...
    const [rows] = await dbPromise.query(
      ...
      WHERE sp.role = ? AND sp.status = 'Active'
        AND LOWER((${PLAN_TYPE_FROM_PLAN_SQL})) != 'enterprise'
      GROUP BY sp.id
      ORDER BY CASE WHEN (${PLAN_TYPE_FROM_PLAN_SQL}) = 'trial' THEN 0 ELSE 1 END, sp.price ASC`,
      [role],
    );
